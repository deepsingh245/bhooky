...
export function FoodCard({ card }: FoodCardProps) {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      {/* Plain deep link, no MCP call — the safety net until real ordering (cart,
          update_food_cart) lands in Phase 2. */}
      <a
        href={swiggyFallbackUrl(restaurant.name)}
        target="_blank"
        rel="noreferrer"
        className="mt-1 rounded-lg bg-orange-500 px-3 py-2 text-center text-sm font-medium text-white"
      >
        Order on Swiggy
      </a>
    </div>
